...
import { CircleMarkerToolbarItem } from './markers/circle/CircleMarkerToolbarItem';
...
export class MarkerArea {
	...
	private setTargetRect = () => {
		...
		this.targetRect = { left: targetRect.left - bodyRect.left, top: targetRect.top - bodyRect.top } as ClientRect;
	};
	...
	private positionToolbar = () => {
		this.toolbarUI.style.left = `${this.targetRect.left + this.target.offsetWidth - this.toolbarUI.clientWidth}px`;
	...
	private positionLogo = () => {
		if (this.logoUI) {
			...
			this.logoUI.style.top = `${this.targetRect.top + this.target.offsetHeight - this.logoUI.clientHeight - 10}px`;
		}
	};
